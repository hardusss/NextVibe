...
export default function CustomChatScreen() {
    ...
    const renderItem = useCallback(
        ({ item, index }: { item: ListItem; index: number }) => {
            ...
            const prevItem = processedListItems[index - 1];
            const nextItem = processedListItems[index + 1];
        ...
        <View style={styles.container}>
            ...
            {loading ? (
                ...
                <View style={styles.keyboardContainer}>
                    ...
                    <View
                        style={[
                            styles.floatingBottomWrapper, 
                            { bottom: keyboardHeight > 0 ? keyboardHeight + (Platform.OS === 'android' ? 24 : 12) : Math.max(insets.bottom, 8) }
                        ]}
                        onLayout={(event) => {
                            const { height } = event.nativeEvent.layout;
                            if (height > 40 && Math.abs(height - inputLayoutHeight) > 2) {
                                inputLayoutHeight !== height && setInputLayoutHeight(height);
                            }
                        }}
                    >
                        <LiquidGlassView
...
const getStyles = (
    ...
    StyleSheet.create({
        ...
        inputWrapper: {
            paddingHorizontal: 12,
            paddingVertical: 6,
            backgroundColor: Platform.OS === 'ios' ? 'transparent' : (isDark ? 'rgba(21, 7, 35, 0.85)' : 'rgba(255, 255, 255, 0.92)'),
        },
        inputInnerContainer: {
            flexDirection: 'row',
            alignItems: 'center',
